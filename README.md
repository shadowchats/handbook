...
## Декомпозиция бизнес-домена на поддомены, за реализацию каждого из которых будет отвечать свой микросервис

### Core
1. Messaging (сообщения - доставка, история и т. д.).
2. Conversations (чаты, группы, каналы и т. д.).

### Supporting
1. User profiles (профили пользователей - имя, аватар, контакты и т. д.).

### Generic
1. Authentication (регистрация; управление токенами - вход и выход).
